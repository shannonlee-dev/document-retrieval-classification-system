...
동일한 전처리·분할에서 진단용 `LinearSVC`를 사용하면 전체 Accuracy는 0.7671에서 0.7851로, 두 클래스의 recall은 각각 0.6490과 0.5041로 상승했다. 이 진단만으로 낮은 성능의 주된 원인을 분리할 수는 없다. 클래스 자체의 의미 중첩, 표본 수 차이, unigram TF-IDF의 feature 제한, 현재 학습 방식은 모두 성능에 기여할 수 있는 가설이다. 직접 구현한 TF-IDF가 scikit-learn과 `1e-6` 이내로 일치한다는 수치 검증과 분류 성능은 서로 다른 검증 대상이다.
...
코사인 유사도는 벡터 사이의 가까움을 나타내는 점수이지 관련 문서일 확률이 아니다. 예를 들어 Top-1 점수 `0.2`를 관련 확률 `20%`로 해석할 수 없다. 절대적인 판단 기준에 가까운 값을 제공하려면 held-out query-document 쌍에 실제 관련 여부 `y ∈ {0, 1}`를 라벨링하고, 다음 로지스틱 보정 모델을 별도로 학습한다.

```text
p(y=1 | x) = sigmoid(βᵀx) = 1 / (1 + exp(-βᵀx))
β̂ = argminβ -Σ[yᵢ log(pᵢ) + (1-yᵢ) log(1-pᵢ)]
```

입력을 `x = [1, cosine_score]`로 제한하면 같은 보정 모델에서 동일한 점수는 항상 동일한 확률을 얻는다. 쿼리별 불확실성을 구분하려면 `Top-1 - Top-2` 점수 차이, 쿼리의 유효 토큰 수, 후보 문서 수처럼 검색 전에 정의할 수 있는 특성을 `x`에 추가하고, 별도 검증 세트에서 calibration error와 Brier score를 확인한다.

계수 추정치의 공분산 행렬을 `Σβ`라고 할 때 점수 `x`의 logit 표준 오차와 95% 신뢰구간은 다음처럼 계산한다. 이 구간은 보정 계수 추정의 불확실성을 나타내며, 개별 문서의 관련 여부가 포함될 범위를 뜻하지 않는다.

```text
η = xᵀβ̂
SE(η) = sqrt(xᵀΣβx)
CI95 = [sigmoid(η - 1.96 × SE(η)), sigmoid(η + 1.96 × SE(η))]
```

충분한 라벨로 학습·검증한 보정 모델이 Top-1 점수 `0.2`를 관련 확률 `78%`, 95% 신뢰구간 `72%–83%`로 산출했다면 출력은 다음과 같이 표시한다. 이 숫자는 출력 형식의 예시이며 현재 산출물에서 측정한 결과가 아니다.
...
보정 확률은 라벨링한 쿼리와 문서 분포를 전제로 한 조건부 확률이므로 보편적인 절대 확률은 아니다. 말뭉치, 쿼리 유형 또는 검색 모델이 바뀌면 calibration 성능을 다시 측정하고 필요하면 모델을 재학습한다.
