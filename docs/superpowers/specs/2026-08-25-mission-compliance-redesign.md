...
새 `privacy.py` 모듈은 프로젝트가 허용하는 일반 주제어 집합을 소유한다. loader에서 받은 원문은 다음 순서로 즉시 변환한다.

1. 영문 소문자 단어를 추출한다.
2. 고정된 일반 주제어 허용 목록에 포함된 토큰만 남긴다.
3. 토큰을 공백으로 연결해 비식별 문서를 만든다.
4. 결과가 빈 문서는 제거한다.
5. 원문은 `DatasetBundle`, 분류 평가, 검색 객체 또는 artifact로 전달하지 않는다.

허용 목록 밖의 토큰은 모두 폐기하므로 이름, 이메일, 전화번호, 주소, 자유 형식 건강정보가 모델 입력과 산출물에 남지 않는다. 허용 목록은 테스트에서 직접 검증한다.

### Dataset model

`DatasetBundle`은 다음 값을 보관한다.

- `texts`: 비식별화된 비어 있지 않은 문서
- `labels`: 연속된 정수 라벨
...
`metadata.json`에는 원문 `texts`를 저장하지 않는다. 다음 값만 기록한다.
...
평가 보고서의 검색 예시와 오분류 예시도 비식별 스니펫만 포함한다.

## Documentation and Licensing

README는 새 문서 수, 세 카테고리, 정제 후 공백 토큰화, 안전 어휘 필터, 빈 문서 제거, NumPy 코사인 연산, NumPy 분류 배치와 변경된 실험 결과를 설명한다.
...
- 원문에서 허용된 일반 주제어만 남긴 파생 데이터를 사용하는 방식
- 보고서와 runtime artifact에는 비식별 파생 텍스트만 저장한다는 정책

## Error Handling

- 정제 후 문서가 500개 미만이거나 카테고리가 두 개 미만이면 build를 중단한다.
- 공백 문서, 길이가 맞지 않는 라벨·문서 ID, 허용 목록 밖 토큰은 검증 오류로 처리한다.
...
2. 개인정보 형태의 입력이 안전 어휘만 남기고 빈 결과를 제거하는 테스트
3. 데이터 loader가 세 카테고리, 500개 이상, 비어 있지 않은 안전 문서만 반환하는 테스트
4. `sparse_dot`이 `np.dot` 기반 기대값과 같은 결과를 내는 테스트
5. 프로젝트와 테스트에 직접적인 SciPy import가 없고 NumPy 배치로 분류되는 테스트
6. 새 artifact가 원문 없이 비식별 스니펫과 문서 ID를 저장·복원하는 테스트
...
- 저장된 텍스트는 비어 있지 않고 안전 어휘로만 구성된다.
